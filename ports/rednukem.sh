...
function sources_rednukem() {
	gitPullOrClone "$md_build"  https://github.com/nukeykt/NBlood.git "master" a4c21eb
}

function build_rednukem() {
    cd $md_build/Rednukem
    make rr STARTUP_WINDOW=0 SDL_TARGET=2 USE_OPENGL=0 USE_LIBVPX=0
	md_ret_require="$md_build"
}

function install_rednukem() {
    md_ret_files=('rednukem'
	'nblood.pk3'
    )
}
...
function configure_rednukem() {
	...
	addPort "$md_id" "rednukem" "Rednukem - Redneck Rampage source port" "$md_inst/rednukem  -j $romdir/ports/rednukem"	
}