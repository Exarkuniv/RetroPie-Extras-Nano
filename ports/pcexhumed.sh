...
function depends_pcexhumed() {
   getDepends nasm libgl1-mesa-dev libglu1-mesa-dev libsdl2-dev libsdl-mixer1.2-dev libsdl2-dev libsdl2-mixer-dev flac libflac-dev libvorbis-dev libvpx-dev libgtk2.0-dev freepats libgl-dev
}
...
function build_pcexhumed() {
    cd $md_build
    make exhumed STARTUP_WINDOW=0 USE_OPENGL=0 USE_LIBVPX=0
	md_ret_require="$md_build"
}

function install_pcexhumed() {
    md_ret_files=('pcexhumed'
	'nblood.pk3'
    )
}
...
function configure_pcexhumed() {
    ...
    addPort "$md_id" "pcexhumed" "PCExhumed - Powerslave source port" "$md_inst/pcexhumed  -j $romdir/ports/pcexhumed"
