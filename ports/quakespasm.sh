...
function sources_quakespasm() {
    gitPullOrClone "$md_build" git://git.code.sf.net/p/quakespasm/quakespasm.git master
}
...
function configure_quakespasm() {
    ...
    addPort "$md_id" "quakespasm" "Quakespasm - Quake engine" "$md_inst/quakespasm -basedir $romdir/ports/quake"
